...
| Симптом | Причина |
|---|---|
| В Issues пусто, в консоли `ERR_BLOCKED_BY_CLIENT` | Блокировщик рекламы режет запросы к Sentry. Отключи для `sentry.io` |
| В Issues пусто, в консоли тишина | Опечатка в DSN. SDK не кричит о неверном DSN — он просто молчит |
